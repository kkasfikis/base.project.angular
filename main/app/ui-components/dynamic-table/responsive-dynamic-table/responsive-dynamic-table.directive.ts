import {
  ...
  @Directive({
    selector: '[responsive-dynamic-table]'
  })
  export class ResponsiveDynamicTableDirective
          ...
          rows.forEach(rowCells =>
            rowCells.forEach(cell =>
              this.renderer.setAttribute(
                cell,
                'data-column-name',
                [...this.thead.rows.item(0)!.children!]
                    .map(headerCell => headerCell.textContent)[cell.cellIndex]!
              )
            )
          )
        );
