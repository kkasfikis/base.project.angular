...
import { FilterField } from "../dynamic-crud/dynamic-crud.models";
...
export abstract class JsonHelpers {
  ...
  public static initNavigatorFromJson(config : any, userType : string){
    let data = JSON.parse(JSON.stringify(config as any)) 
    let navbar = data[userType + 'Navbar'];
    let drawer = data[userType + 'Drawer'];
    return {
      navbar : navbar,
      drawer : drawer
    }
  ...
  public static setSubFieldValue(formFields: (BehaviorSubject<FormFieldBase> | BehaviorSubject<SubForm>)[], subformKey: string, field: string, value: any) {
  ...
  public static setSubFieldDropdown(formFields: (BehaviorSubject<FormFieldBase> | BehaviorSubject<SubForm>)[], subformKey: string, fields: string[], values: any[][]) {
    let subForm = formFields.find(x => x.getValue().key == subformKey) as BehaviorSubject<SubForm>;
    let subFormValue = subForm?.getValue() as SubForm
