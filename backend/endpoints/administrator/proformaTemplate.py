...
class GetPostProforma(Resource):
    ...
    @cross_origin()
    def post(self):
        return BaseCrud.create('ProformaTemplate',json.loads(request.form.to_dict()['data']), request.files.to_dict())
...
class PutDeleteProforma(Resource):
    ...
    @cross_origin()
    def put(self, id):
        return BaseCrud.update('ProformaTemplate', id, json.loads(request.form.to_dict()['data']), request.files.to_dict())
