...
class GetPostVessel(Resource):
    ...
    @cross_origin()
    def post(self):
        return BaseCrud.create('Vessel', json.loads(request.form.to_dict()['data']), request.files.to_dict())
...
class PutDeleteVessel(Resource):
    ...
    @cross_origin()
    def put(self, id):
        return BaseCrud.update('Vessel', id, json.loads(request.form.to_dict()['data']), request.files.to_dict())
