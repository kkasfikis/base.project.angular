...
api = Namespace('userManagement',description = 'User Crud Endpoints')
...
class GetPostUserManagement(Resource):
    ...
    @cross_origin()
    def post(self):
        return BaseCrud.create('User',request.get_json())
...
class PutDeleteUserManagement(Resource):
    ...
    @cross_origin()
    def put(self, id):
        return BaseCrud.update('User', id, request.get_json())
